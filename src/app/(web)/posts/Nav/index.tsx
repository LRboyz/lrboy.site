...
import { Navigate, navigitions } from '@/constants/navigation'
...
function NavItem({ nav, href }: { nav: Navigate; href?: string }) {
  ...
  const isActived = nav.label === category

  return (
    <Link
      href={href ? href : `/posts?category=${nav.label}`}
      className={cn(
        'flex px-4 py-2 my-1 hover:bg-zinc-200/50 dark:hover:bg-zinc-800  cursor-pointer duration-100 transition gap-2 items-center',
        {
          'bg-zinc-200/50 dark:bg-zinc-800': isActived
        }
      )}
    >
